...
echo ""
echo "  bash scripts/test.sh tests/llm/test_zhipu.py            # 指定测试文件"
echo "  bash scripts/test.sh tests/llm/test_zhipu.py::TestZhipuGenerate          # 指定测试类"
echo "  bash scripts/test.sh tests/llm/test_zhipu.py::TestZhipuGenerate::test_generate_simple_response  # 指定测试函数"
echo ""
echo "  bash scripts/test.sh -k \"zhipu\"                       # 使用关键字匹配测试"
echo "  bash scripts/test.sh tests/llm/ -x -s                   # -x 遇到失败停止, -s 显示输出"
